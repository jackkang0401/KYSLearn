    ...
    int maxProfit(int k, vector<int>& prices) {
        /*
            一、DP 数组以及下标的含义
                ...
                dp[i][k][c]：i 表示天数；k 表示交易次数；c 表示持有股票份数，值为 [0,1]
            ...
            二、DP 方程
                ...
                dp[i][j][1] = max(dp[i-1][j][1], dp[i-1][j-1][0]-prices[i]);    // 持有 1 份股票(买过) 
                dp[i][j][0] = max(dp[i-1][j][0], dp[i-1][j][1]+prices[i]);      // 持有 0 份股票(已卖) 
        */
        int size = prices.size();
        if (size == 0) return 0;
